...
the first search constructs a list of nodes according to the structure of the
graph, and the second search forms the strongly connected components.
*/
...
struct Scc {
  ...
  void DFS(int u) {
    ...
    for (auto v : E[u])
      if (!vst[v]) DFS(v);
    vec.PB(
        u);  // Each node will be added to the list after it has been processed.
    // std::cout << "u=" << u+1 << '\n';
  }
  void rDFS(int u) {
    vst[u] = 1;
  ...
  void solve() {
    ...
    // that during the second search, we will always find strongly connected
