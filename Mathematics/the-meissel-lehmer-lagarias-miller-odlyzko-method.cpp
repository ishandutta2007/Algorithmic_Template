...
void init_phiTable() {
  ...
  for (i = 0; i < Q; ++i) phiTable[i] = i;

  for (i = 1; i <= ST; ++i) {
    p = Prime[i];
    for (j = Q - 1; j >= 0; --j) phiTable[j] -= phiTable[j / p];
  }
}
...
void init_minFactor() {
  ...
  for (i = 0; i < maxy; i++) minfactor[i] = i;
  ...
  for (i = 1; Prime[i] <= sqrty; i++) {
    ...
    for (j = powp; j <= maxy; j += powp) minfactor[j] = 0;
  }
}
...
int sieve() {
  ...
  for (int p = 3; p < maxp; p += 2) {
    if (!(bitMask[p >> COMP] & MASKN(p))) {
      Prime[++primes] = p;
      if (p > maxp / p) continue;
      for (int j = p * p; j < maxp; j += p << 1) bitMask[j >> COMP] |= MASKN(j);
    }
  }
  Prime[0] = primes;
  Prime[primes] = 0;
