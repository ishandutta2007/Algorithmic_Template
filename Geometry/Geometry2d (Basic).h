...
bool LineInter(const point& A, const point& B, const point& C, const point& D,
               point& P) {
  point AB(B - A), CD(D - C);
  if (sgn(AB.det(CD)) == 0) return false;  // parallel
  P = A + AB * (CD.det(C - A) / CD.det(AB));
...
flt DisSeg2Seg(const point& A, const point& B, const point& C, const point& D) {
  point O;
  if (SegInter(A, B, C, D, O))
    return 0;
  else
    return std::min(std::min(Dis2Seg(A, B, C), Dis2Seg(A, B, D)),
...
bool intersect(const point& A, flt ra, const point& B, flt rb, point& P,
               point& Q) {
  point AB(B - A);
  double dis = AB.norm();
...
bool intersect(const point& A, const point& B, const point& O, flt r, point& P,
               point& Q) {
  point AB(B - A), AO(O - A);
