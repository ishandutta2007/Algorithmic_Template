...
struct Point {
  ...
  Point operator*(const flt k) const { return Point(x * k, y * k, z * k); }
  Point operator/(const flt k) const { return Point(x / k, y / k, z / k); }
  flt dot(const Point& rhs) const { return x * rhs.x + y * rhs.y + z * rhs.z; }
  Point det(const Point& rhs) const {
    return Point(y * rhs.z - z * rhs.y, z * rhs.x - x * rhs.z,
                 x * rhs.y - y * rhs.x);
  }
  flt sqrlen() const { return x * x + y * y + z * z; }
  flt len() const { return sqrt(sqrlen()); }
...
// 判断直线AB和CD是否平行
bool is_parallel(const Point& A, const Point& B, const Point& C,
                 const Point& D) {
