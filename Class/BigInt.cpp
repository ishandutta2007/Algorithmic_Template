...
void fft(vector<complex<double> >& y, int on) {
  ...
  if (on == -1)
    for (auto& i : y) i.real(i.real() / len);
}
class BigInt {
 private:
  string num;
 ...
 public:
  ...
  friend bool operator>(const BigInt& t, const BigInt& s) { return s < t; }
  friend bool operator==(const BigInt& t, const BigInt& s) {
    return t.num == s.num && t.sign == s.sign;
  }
  friend bool operator!=(const BigInt& t, const BigInt& s) { return !(t == s); }
  friend bool operator<=(const BigInt& t, const BigInt& s) {
    return t == s || t < s;
  }
  friend bool operator>=(const BigInt& t, const BigInt& s) {
  ...
  friend BigInt& operator+=(BigInt& t, const BigInt& s) { return t = t + s; }
  friend BigInt& operator-=(BigInt& t, const BigInt& s) { return t = t - s; }
  friend BigInt& operator*=(BigInt& t, const BigInt& s) { return t = t * s; }
  friend BigInt& operator/=(BigInt& t, const BigInt& s) { return t = t / s; }
  friend BigInt& operator%=(BigInt& t, const BigInt& s) { return t = t % s; }
  ...
  const BigInt subnum(int l) { return this->subnum(0, l); }
};
BigInt dp[110][110];
